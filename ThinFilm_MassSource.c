...
ThinFilmMassSource.c
    ...
    Ths is a User Defined Function (UDF) to simulate     This UDF computes the massFlux generated by the thin film to add as a 
    source term to the ANSYS Fluent Simulation

    Developed By: Saaras Pakanati, Ayaaz Yasin, and Kishan Bellur - Oct 2025
    ...
    --------------------------------------------------------------------------------------
    KL                      Liquid Thermal Conductivity Curve Fit                   double
    KL                      Vapor Thermal Conductivity Curve Fit                    double
    PV                      Vapor Pressure                                          double
    sigma                   Surface Tension                                         double
    RL                      Liquid Density                                          double
    RV                      Vapor Density                                           double
    Hfg                     Latent Heat of Vaporization                             double
    mu                      Viscosity                                               double

    preCondition Function
    --------------------------------------------------------------------------------------
    HaC                     Hamacker Constant                                       double
    zeta                    Under-relaxation factor                                 double
    R                       Radius                                                  double
    M                       Molar Mass                                              double
    mB                      Molecular Mass                                          double

    derivativeFunction Function
    --------------------------------------------------------------------------------------
    R                       Radius                                                  double
    HaC                     Hamacker Constant                                       double
    gamma                   Surface Tension, Temperature derivative                 double

    thinFilmSolve Function
    --------------------------------------------------------------------------------------
    x                       Initial location                                        double
    L                       Interface length                                        double
    dx                      Spatial discretization                                  double
    h0                      Initial height                                          double
    h1                      First Height Derivative                                 double
    h2                      Second Height Derivative                                double
    M_lastEV                Inlet Mass Flow                                         double
    R                       Radius                                                  double
    H_ad                    Adsorbed Film Height [Optional stopping condition]      double
   
    DEFINE_SOURCE Function
    --------------------------------------------------------------------------------------
    length                  Number of cells the wall temperature UDF provides       double
    Tv_main                 Saturation Temperature                                  double
    tune                    Gamma Formulation for Knudsen Layer Reduction           double
...
// getAlpha FUNCTION
//      provides the accomodation coefficient based on the transition state theory based equation.
...
double getAlpha(double RV, double RL)
{
    ...
    // a = 0.58;                        // <---------- If you want to override and use a constant value.
...
double preCondition(double A, double B, double x, int iter, double h, double h1, double h2, double T_lastEv, double Tv_main, double tune, double* Ti, double* mFlux) 
{ 
    ...
    double Tv = Tv_main * (1 - tune);               // [K] Vapor Temperature.
...
double derivativeFunction(double dx, double h, double h1, double h2, double k, double A, double B, double x, int iter, double Tv_main, double tune, double *Hppp, double *T_lastEv, double *M_lastEv, double *mFlux) 
{
    
    // Initializing and Computing the Interfacial Temperature and Mass Flux.
    double Ti;
    preCondition(A, B, x, iter, h, h1, h2, *T_lastEv, Tv_main, tune, &Ti, mFlux);
    ...
    double ds_dx    = gamma * dT_dx;                                // [] Surface Tension Gradient
...
double thinFilmSolve(double A, double B, double vol, int iter, double Tv_main, double tune) 
{
    ...
    preCondition(A, B, x, iter, h0, h1, h2, T_lastEv, Tv_main, tune, &Ti, &mFlux);
    T_lastEv = Ti;
    Ti_last = Ti;
    ...
    for (int  j = 0; j < n; j++) {
        ...
        if (mFlux > 1e-5) {
            // Initializer Step
            derivativeFunction(dx, H[0], H[1], H[2], 0.0, A, B, x, iter, Tv_main, tune, &Hppp, &T_lastEv, &M_lastEv, &mFlux);
            T_lastEv = Ti_last;
            M_lastEv = M_last;
            ...
            derivativeFunction(dx, H[0], H[1], H[2], 0.0, A, B, x+(dx/2), iter, Tv_main, tune, &Hppp, &T_lastEv, &M_lastEv, &mFlux);
            T_lastEv = Ti_last;
            M_lastEv = M_last;
            ...
            derivativeFunction(dx, H[0], H[1], H[2], 0.0, A, B, x+dx, iter, Tv_main, tune, &Hppp, &T_lastEv, &M_lastEv, &mFlux);
        ...
        // Extracting alpha for reference.
        double KL, KV, PV, sigma, RL, RV, Hfg, mu;
...
DEFINE_SOURCE(ThinFilmMassSource,c,t,dS,eqn)
{
    ...
    double Sigma_Y = 0;
    int length = 19;                           // # of cells reading for wall temperature.
    ...
    double tune = 0.00032;                      // Gamma Formulation
    ...
    if (iter > 100) {
        ...
        if (iter % 5 == 0.0) {
            ...
            Sm = thinFilmSolve(A, B, vol, iter, Tv_main, tune);
            Sm_Global = Sm;
        } else {
